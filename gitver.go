...
func DetectVersionWithOpts(rootDir string, opts *VersioningOpts) (*VersionInfo, error) {
	...
	// Open repo once — shared for SHA, branch, tag, and fetch operations.
	repo, err := gitstate.OpenRepo(rootDir)
	if err != nil {
		return nil, fmt.Errorf("opening repo: %w", err)
	}

	// Current SHA (short, 7 chars)
	head, err := repo.Head()
	if err != nil {
	...
	// Current branch (returns "HEAD" on detached — same as --abbrev-ref behaviour)
	...
	//
	// WARNING:
	// gitstate.ListTagsSorted uses semver-aware sorting (Masterminds/semver) to
	// match git's --sort=-v:refname order. Non-semver tags sort lexicographically
	// after semver tags. StageFreight WILL NOT reinterpret or re-sort tags.
	tags, _ := gitstate.ListTagsSorted(repo)
	if len(tags) == 0 {
		// No local tags — try fetching (handles shallow clones and CI environments
		// that don't fetch tags by default). Auth is resolved from environment/SSH
		// agent so this works in both CI and local dev.
		originURL, _ := gitstate.RemoteURL(repo, "origin")
		auth, _ := gitstate.ResolveAuthForURL(originURL)
		_ = repo.Fetch(&git.FetchOptions{Tags: git.AllTags, Depth: 1, Auth: auth})
		tags, _ = gitstate.ListTagsSorted(repo)
	}
...
func headAtTag(repo *git.Repository, tag string) bool {
	exact, err := gitstate.ExactTagAtHEAD(repo)
	if err != nil {
		return false
	}
	return exact == tag
