...
// Uses go-git with DetectDotGit to walk parent directories, matching git CLI behaviour.
...
func checkGitClean(_ context.Context, repoRoot string) error {
	...
	if gitstate.HasUnstagedChanges(status) {
		paths := gitstate.UnstagedDirtyPaths(status)
		sort.Strings(paths)
		indented := make([]string, len(paths))
		for i, p := range paths {
			indented[i] = "  " + p
		}
		return fmt.Errorf("tracked files have unstaged changes:\n%s", strings.Join(indented, "\n"))
	}
	if gitstate.HasStagedChanges(status) {
		paths := gitstate.StagedFiles(status)
		sort.Strings(paths)
		indented := make([]string, len(paths))
		for i, p := range paths {
			indented[i] = "  " + p
...
// Reads the git index directly — equivalent to `git ls-files`.
