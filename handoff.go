...
// Uses go-git remote listing — no git binary needed.
func resolveRemoteHead(branch string) string {
	cwd, err := os.Getwd()
	if err != nil {
		return ""
	}
	repo, err := gitstate.OpenRepo(cwd)
	if err != nil {
		return ""
	}
	session, err := gitstate.OpenSyncSession(cwd)
	if err != nil {
		return ""
	}
	hash, err := gitstate.RemoteRefHash(repo, "origin", branch, session.Auth())
	if err != nil {
		return ""
	}
	return hash.String()
