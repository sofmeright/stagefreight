...
// two-file config merge, governance reconciliation, capability detection,
// and execution gating.
//
// Preset resolution is defined in src/config/preset — it is a manifest-level
// feature consumed by governance, not owned by it.
package governance
...
// MergeEntry and MergeTrace are defined in src/config/preset.
// They are re-exported here for callers that already import governance.
// New callers should import github.com/PrPlanIT/StageFreight/src/config/preset directly.
type MergeEntry = preset.MergeEntry
type MergeTrace = preset.MergeTrace
